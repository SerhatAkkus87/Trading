...
# Define the rolling window
    ...
    df.loc[:, 'high_roll_max'] = df.loc[:, 'High'].rolling(window=roll_window).max()
    df.loc[:, 'low_roll_min'] = df.loc[:, 'Low'].rolling(window=roll_window).min()
    ...
    df.loc[:, 'head_shoulder_pattern'] = np.nan
