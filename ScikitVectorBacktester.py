...
class ScikitVectorBacktester(object):
    ...
    def __init__(self, symbol, start, end, amount, tc, model, raw_data):
        self.symbol = symbol
        self.start = start
        self.end = end
        self.amount = amount
        ...
        if model == 'regression':
            ...
            self.model = linear_model.LogisticRegression(C=1e6,
                solver='lbfgs', multi_class='ovr', max_iter=1000, tol=1e-6)
        else:
    ...
    def run_strategy(self, start_in, end_in, start_out, end_out, lags=3):
        ...
        hits = np.sign(self.data_subset['prediction']).value_counts()
